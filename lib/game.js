...
class Game {
  ...
  run() {
    ...
    self.players.push(new Player({ x: 151, y: 150, color: 'blue', controls: 'wasd' }, self.map))

    self.flags.push(new Flag(self.map.blueFlagOptions, self.map))
    self.flags.push(new Flag(self.map.redFlagOptions, self.map))
...
window.onload = function() {
  ...
  var map = new Map(new MapBlueprint().level_one) // indicate map to use
